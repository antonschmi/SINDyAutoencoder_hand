...
def library_size(n, poly_order, use_sine=False, include_bias=True):
    """Helper to replace the imported sindy_utils.library_size"""
    l = 0
    for p in range(poly_order + 1):
        l += math.comb(n + p - 1, p)
    if use_sine:
        l += 2 * n
    return l if include_bias else l - 1
